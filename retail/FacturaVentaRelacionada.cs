...
namespace VID_Retail.FacturaVentaRelacionada
{
    class TFacturaVentaRelacionada : TvkBaseForm, IvkFormInterface
    {
        ...
        public TFacturaVentaRelacionada(List<object> oForms)
        {
            R_oForms = oForms;
        }
        ...
        public new bool InitForm(string uid, string xmlPath, ref SAPbouiCOM.Application application, ref SAPbobsCOM.Company company, ref CSBOFunctions sboFunctions, ref TGlobalVid _GlobalSettings)
        {
            ...
            try
            {
                ...
                oBtn.Caption = "Insertar en Gerona";
                ...
                switch (pVal.EventType)
                {
                    case BoEventTypes.et_CLICK:
                        if ((pVal.ItemUID == "btnInsCR") && (pVal.BeforeAction))
                            BubbleEvent = (1 == FSBOApp.MessageBox("¿Desea enviar documento a venta relacionada?", 2, "Ok", "Cancel"));
                        else if ((pVal.ItemUID == "btnInsCR") && (!pVal.BeforeAction))
                        {
                            if ("" == oForm.DataSources.DBDataSources.Item("OINV").GetValue("FolioNum", 0).Trim())
                            {
                                FSBOApp.StatusBar.SetText("Documento sin folio.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                                break;
                            }
                            if ("" == CreateFaturaCompraGerona(oForm.DataSources.DBDataSources.Item("OINV"), oForm.DataSources.DBDataSources.Item("INV1")))
                            {
                                R_application.MessageBox("Factura ingresada", 1, "Ok", "", "");
                                oTxt.Caption = "Factura en Gerona: " + oForm.DataSources.DBDataSources.Item("OINV").GetValue("U_VR_DocRel", 0).Trim();
                                oBtn.Item.Visible = false;
                            }
                        }
                        break;
            ...
            try
            {
                switch (oBusinessObjectInfo.EventType)
                {
                    case BoEventTypes.et_FORM_DATA_LOAD:
                        ...
                        {
                            if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("DocType", 0).Trim() != "I")
                            {
                                oTxt.Caption = "";
                                oBtn.Item.Visible = false;
                            }
                            else if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("CardCode", 0).Trim() != GlobalSettings.SNGerona)
                            {
                                oTxt.Caption = "";
                                oBtn.Item.Visible = false;
                            }
                            else if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("U_VR_DocRel", 0).Trim() == "")
                            {
                            ...
                            {
                                oTxt.Caption = "DocNum en Gerona: " + oForm.DataSources.DBDataSources.Item("OINV").GetValue("U_VR_DocRel", 0).Trim();
                                oBtn.Item.Visible = false;
                            }
                        }
                        break;
                    case BoEventTypes.et_FORM_DATA_ADD:
                        ...
                        {
                            if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("DocType", 0).Trim() == "I")
                                if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("CANCELED", 0).Trim() == "N")
                                    if (oForm.DataSources.DBDataSources.Item("OINV").GetValue("CardCode", 0).Trim() == GlobalSettings.SNGerona)
                                        resetDocRel(oForm.DataSources.DBDataSources.Item("OINV").GetValue("DocEntry", 0));
                        }
                        break;
                }
            }
        ...
        private void resetDocRel(String oDocEntry)
        {
            ...
            oSql = GlobalSettings.RunningUnderSQLServer ?
                  "Update OINV set U_VR_DocRel = NULL where DocEntry = {0} " :
                  "Update OINV set \"U_VR_DocRel\" = NULL where \"DocEntry\" = {0} ";

            oRS.DoQuery(String.Format(oSql, oDocEntry.Trim()));
        }

        private String CreateFaturaCompraGerona(SAPbouiCOM.DBDataSource DSHead, SAPbouiCOM.DBDataSource DSDet)
        {
            String sErr = "";
            Int32 oDocEntry;
            ...
            try
            {
                SAPbobsCOM.Documents oDoc = (SAPbobsCOM.Documents)GlobalSettings.oCompanyVentaRelacionada.GetBusinessObject(BoObjectTypes.oPurchaseInvoices);
                ...
                oDoc.Comments = "Factura CR relacionada - " + DSHead.GetValue("DocNum", 0) + "  " + DSHead.GetValue("Comments", 0);
                ...
                for (int i = 0; i <= DSDet.Size - 1; i++)
                {
                    ...
                    oDoc.Lines.UnitPrice = Double.Parse(DSDet.GetValue("PriceBefDi", i), System.Globalization.CultureInfo.InvariantCulture);
                ...
                if (GlobalSettings.Debug)
                    oDoc.SaveToFile("oPurchaseInvoice.xml");
                int nErr = oDoc.Add();
                if (nErr != 0)
                {
                ...
                {   // Update este documento local
                    ...
                    SAPbobsCOM.Documents esteDoc = (SAPbobsCOM.Documents)FCmpny.GetBusinessObject(BoObjectTypes.oInvoices);
